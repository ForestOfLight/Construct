...
class BuildBlockModelsTest(unittest.TestCase):
    ...
    def _pitcher_crop_mcmeta(self):
        # Mirrors pitcher_crop: its upper half is a real block from the moment
        # the crop is planted, but the plant only grows tall enough to reach
        # it at age 3 - so Mojang ships block/pitcher_crop_top_stage_0 with
        # textures and no elements, and the age 3 model with geometry.
        return FakeMcmeta(
            blockstates={"pitcher_crop": {"variants": {
                "age=0,half=upper": {"model": "block/pitcher_crop_top_stage_0"},
                "age=3,half=upper": {"model": "block/pitcher_crop_top_stage_3"},
            }}},
            models={
                "block/pitcher_crop_top_stage_0": {"textures": {}},
                "block/pitcher_crop_top_stage_3": {"textures": {}, "elements": [{
                    "from": [4, 0, 4], "to": [12, 16, 12],
                    "faces": {"up": {"texture": "block/pitcher_crop_top"}},
                }]},
            },
        )

    def test_a_state_java_draws_nothing_for_gets_no_faces_not_a_missing_cube(self):
        # The empty model is Java's own answer, not a gap in our data, so the
        # missing-block cube would advertise a problem that doesn't exist.
        atlas = FakeAtlas()
        b2j = {
            "minecraft:pitcher_crop[growth=0,upper_block_bit=1]":
                "minecraft:pitcher_crop[age=0,half=upper]",
            "minecraft:pitcher_crop[growth=5,upper_block_bit=1]":
                "minecraft:pitcher_crop[age=3,half=upper]",
        }
        block_models = build_block_models(self._pitcher_crop_mcmeta(), b2j, atlas)
        self.assertEqual(block_models["minecraft:pitcher_crop[growth=0,upper_block_bit=1]"], [])
        # the state that does have geometry is untouched
        self.assertEqual(len(block_models["minecraft:pitcher_crop[growth=5,upper_block_bit=1]"]), 1)

    def test_an_empty_model_stays_a_missing_cube_when_no_state_has_geometry(self):
        # block/skull is the same shape of file as pitcher_crop's empty top -
        # a textures dict and no elements - but a skull is a block entity
        # Java draws in code and we genuinely have no model for. What tells
        # them apart is that no state of a skull has geometry, so dropping it
        # silently would hide a block we still need to model.
        atlas = FakeAtlas()
        mcmeta = FakeMcmeta(
            blockstates={"skeleton_skull": {"variants": {
                "rotation=0": {"model": "block/skull"},
                "rotation=1": {"model": "block/skull"},
            }}},
            models={"block/skull": {"textures": {}}},
        )
        b2j = {
            "minecraft:skeleton_skull[rot=0]": "minecraft:skeleton_skull[rotation=0]",
            "minecraft:skeleton_skull[rot=1]": "minecraft:skeleton_skull[rotation=1]",
        }
        block_models = build_block_models(mcmeta, b2j, atlas)
        self.assertEqual(block_models["minecraft:skeleton_skull[rot=0]"], WHITE_CUBE_FACES)

    def test_a_later_state_proving_the_block_is_modelled_still_empties_an_earlier_one(self):
        # The empty state is reached before the one with geometry, so the
        # decision can only be made once every state has been seen.
        atlas = FakeAtlas()
        b2j = {
            "minecraft:pitcher_crop[growth=0,upper_block_bit=1]":
                "minecraft:pitcher_crop[age=0,half=upper]",
        ...
        self.assertEqual(list(b2j)[0], "minecraft:pitcher_crop[growth=0,upper_block_bit=1]")
        block_models = build_block_models(self._pitcher_crop_mcmeta(), b2j, atlas)
        self.assertEqual(block_models["minecraft:pitcher_crop[growth=0,upper_block_bit=1]"], [])
