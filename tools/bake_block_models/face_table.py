...
from face_geometry import billboard_facing
from neighbor_culling import covers_side, cull_direction
...
def build_face_types_and_refs(block_models):
    ...
    Call this after atlas_uv.project_uv, so faces carry a 'uv' rect rather
    than a texture name."""
    face_type_index = {}
    face_types = []
...
def _describe(face):
    ...
    # or down (see face_geometry.billboard_facing) - reading the cull
    ...
    if covers:
        ...
        # here; the opacity half was settled by neighbor_culling.
