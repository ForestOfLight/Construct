...
~~#23: Hanging signs are rendered as a missing block in every permutation.~~ Nothing was wrong with the models - the lookup never reached them. Bedrock keeps a hanging sign's direction in two different states and reads whichever its mounting calls for: `ground_sign_direction` when it hangs from a ceiling by chains (`attached_bit=1`), `facing_direction` otherwise, both free-hanging and wall-mounted. The state it isn't reading stays 0. blocksB2J is keyed by Java coverage - one Bedrock permutation per Java state, 36 of the 384 Bedrock has - and fills the unread states with whatever the Java->Bedrock direction produces: a `facing_direction` on ceiling signs, `attached_bit=1` and a `ground_sign_direction` on wall signs. A live sign carries none of that, and `BlockModelLookup` requires every state the data discriminates on to match, so all 384 permutations of all 12 sign types fell through to the missing cube. `rekey_hanging_signs` in main.py now rewrites those entries to name only the states Bedrock reads, which the lookup's partial match finds whatever the rest hold. The four unattached ceiling rotations come from the wall entries, which are the only place the data says which rotation a `facing_direction` means. The permutations still left unmatched are the ones with `facing_direction` 0 or 1 - down and up - which no sign can be placed at.
